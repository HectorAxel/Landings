...
(function ($) {
  $.scrollSwap = function (el, options) {
    ...
    base.init = function () {
      ...
      base.data = [];
      base.isMobile = /Android|webOS|iPhone|iPod|BlackBerry/.test(
        navigator.userAgent
      );
    ...
    base.findBreakPoints = function () {
      ...
      base.trueHeight = base.isMobile
        ? window.innerHeight * base.options.triggerFromTop
        : $(window).height() * base.options.triggerFromTop;

      //empty out the data variable
      base.data.length = 0;
      base.$el
        .find("[ data-" + base.options.dataImageAttribute + "]")
        .each(function (i, d) {
          base.data.push({
            elm: $(d),
            image: $(d).data("image"),
            offset: Math.round($(d).offset().top),
          });
        });
    };
      ...
      $("> *", base.$el).each(function (i, d) {
        ...
        if (base.options.noCSS == false) {
          $(d).css({
            ...
            marginTop: 0,
            marginBottom: $(document).height() / 9,
        ...
        if (
          typeof $(d).data("position") == "undefined" ||
          $(d).data("position").toLowerCase() == "center"
        ) {
          $(d).css({
            marginLeft: "auto",
            marginRight: "auto",
        ...
        if (
          typeof $(d).data("position") != "undefined" &&
          $(d).data("position").toLowerCase() == "left"
        ) {
          $(d).css({
            marginLeft: "5%",
            marginRight: "auto",
        ...
        if (
          typeof $(d).data("position") != "undefined" &&
          $(d).data("position").toLowerCase() == "right"
        ) {
          $(d).css({
            marginLeft: "auto",
            marginRight: "5%",
    ...
    return this.each(function () {
      ...
      $(window).on("scroll", function () {
        ...
        scroll.data.forEach(function (d, i) {
          ...
          if (
            scrollTop - scroll.trueHeight + $(window).height() <
            scroll.data[0].offset
          ) {
          ...
          //if it's ON the first box
            ...
            scrollTop - scroll.trueHeight + $(window).height() >
              scroll.data[0].offset &&
            scrollTop < scroll.data[1].offset
          ) {
          ...
          //if it's after the last one
            ...
            scrollTop >
            scroll.data[scroll.data.length - 1].offset +
              $(window).height() * scroll.options.triggerFromTop
          ) {
            $(".scrollBackgroundHolder").last().css({ opacity: 0 });
          }

          //if it's ON the last text box
          else if (scrollTop > scroll.data[scroll.data.length - 1].offset) {
            $(".scrollBackgroundHolder")
              .not(":nth-child(" + (scroll.data.length - 1) + ")")
              .css("opacity", 0);
          ...
          //if it's between text boxes
            ...
            scrollTop > scroll.data[i].offset &&
            scrollTop < scroll.data[i + 1].offset
          ) {
            $(".scrollBackgroundHolder")
              .not(":nth-child(" + i + ")")
              .css("opacity", 0);
